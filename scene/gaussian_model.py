#定义全局的高斯模型，其实就是一个类
#值得注意的是，我们做的是CT重建的工作，所以高斯元只有中心位置X、协方差矩阵、密度
import torch
...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        self.optimizer_4d = torch.optim.Adam(l_4d, lr=0.0, eps=1e-15)
    ...
    def add_point_by_mask(self, selected_pts_mask, perturb=0):
        ...
        new_densities = self._density[selected_pts_mask][mask]
        ...
        self.densification_postfix(new_xyz, new_densities, new_scaling, new_rotation, new_deformation_table)
    ...
    def standard_constaint(self):
        ...
        density = self._density.detach()
        time =  torch.tensor(0).to("cuda").repeat(means3D.shape[0],1)
        means3D_deform, scales_deform, rotations_deform, _ = self._deformation(means3D, scales, rotations, density, time)
