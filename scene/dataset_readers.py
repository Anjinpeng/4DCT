...
class SceneInfo(NamedTuple):
    train_cameras_4d: list
    test_cameras_4d: list
    vol_4d: list 
    scanner_cfg: dict
    scene_scale: float

def readBlenderInfo(path_file, eval):
    ...
    for idx , file_name in enumerate(all_files):
        path=osp.join(path_file, file_name)
        ...
        cam_infos = readCTameras(meta_data, path, eval, scene_scale,idx,max_idx)
        train_cam_infos = cam_infos["train"]
        test_cam_infos = cam_infos["test"]
        vol_gt = torch.from_numpy(np.load(meta_data["vol"])).float().cuda()
    
        train_cam_infos_4d.append(train_cam_infos)
        test_cam_infos_4d.append(test_cam_infos)
        vol_gt_4d.append(vol_gt)
    
    scene_info = SceneInfo(
        train_cameras_4d=train_cam_infos_4d,
        test_cameras_4d=test_cam_infos_4d,
...
def readCTameras(meta_data, source_path, eval=False, scene_scale=1.0,idx=0,max_idx=9):
    """Read camera info."""
